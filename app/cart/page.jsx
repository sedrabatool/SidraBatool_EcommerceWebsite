    ...
    <div className="font-custom">
      ...
      <div className="flex gap-20 mb-60">
            ...
            {isLoading ? (
              ...
              items.map((item) => (
                <div key={item.id} className="flex items-center mt-6 gap-2 w-[100%]">
                  ...
                  <div className="h-8 w-8 rounded border-2 text-center ml-[200px]">{item.count}</div>
                  <p className="ml-[180px]">Rs. {(parseFloat(item.price.replace(/\D/g, '')) * item.count).toLocaleString()}</p>
        ...
        <div className="bg-[#F9F1E7] w-[30%] h-[50%] p-5 mt-20">
          ...
          <div className="flex flex-col gap-8 px-[150px]">
            <p className="mt-20 font-semibold text-2xl ">Subtotal <span className="text-[#9F9F9F] ">   Rs. {total.toLocaleString()}</span></p>
            <p className='font-semibold text-2xl'>Total <span className="text-[#B88E2F] text-lg  ">Rs. {total.toLocaleString()}</span></p>
